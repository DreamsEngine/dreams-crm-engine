...
export function ConversationPicker({
	...
	const label = current?.title ?? t("agentPanel.newConversation");
						...
						conversations.map((conversation) => (
							...
							>
								<span className="min-w-0 flex-1 truncate">
									{conversation.title ?? t("agentPanel.untitled")}
								</span>
...
function Forget({
	...
	const remove = useMutation(
		trpc.conversations.remove.mutationOptions({
			onSuccess: async () => {
				...
				onDone();
				toast.success(t("agentPanel.conversationDeleted"));
			},
	...
	return (
		...
		>
			<Icon icon={TrashCan} />
			<span className="sr-only">{t("agentPanel.deleteConversation")}</span>
		</Button>
	);
}

...
function currencyOptions(currency: string, t: Translate) {
	...
	return [
		{
			value: currency,
			label: t("deal.currencyNoLongerSupported", { currency }),
		},
		...CURRENCY_OPTIONS,
	];
}
...
function ReportedValue({ deal }: { deal: Deal }) {
	...
	return (
		<DetailSheetProperty
			label={t("deal.inCurrency", { currency: deal.reportingCurrency })}
		>
			{deal.baseAmountCents === null ? (
				<span className="text-muted-foreground">
					{t("deal.noRate", { currency })}
				</span>
			) : (
...
function contactColumns(t: Translate) {
	return [
		...
		{ id: "role", header: t("contact.dealsColumnRole"), width: "w-[20%]" },
		{ id: "title", header: t("common.title"), width: "w-[22%]" },
		{ id: "email", header: t("common.email"), width: "w-[22%]" },
		{ id: "remove", srLabel: t("deal.removeContact"), width: "w-10" },
	];
}
...
export function DealSheet({ dealId }: { dealId: string }) {
	...
	return (
		<RecordSheetFrame
			...
			title={deal?.name ?? t("deal.defaultTitle")}
			description={
				deal ? (
					<button
						type="button"
				...
				deal ? (
					<>
						...
						<RecordActions
							...
							name={deal.name}
							consequence={t("deal.consequence", {
			...
			stats={
				deal ? (
					<DetailSheetStats>
						<DetailSheetStat label={t("common.amount")}>
						...
						<DetailSheetStat label={t("deal.expectedClose")}>
...
function DealOverview({ deal }: { deal: Deal }) {
	...
	return (
		<DetailSheetBody>
			<DetailSheetSection title={t("deal.stageTitle")}>
				<StageStepper dealId={deal.id} stage={deal.stage} />

				{deal.closedReason ? (
					<DetailSheetProperties>
						<DetailSheetProperty label={t("deal.closed")}>
							{deal.closedAt ? (
						...
						<DetailSheetProperty label={t("deal.reason")} wide>
							{deal.closedReason}
						</DetailSheetProperty>
					</DetailSheetProperties>
				) : null}
			...
			<DetailSheetSection title={t("deal.descriptionTitle")}>
				<InlineTextArea
					label={t("deal.descriptionTitle")}
...
function WhereItStands({ deal }: { deal: Deal }) {
	...
	return (
		<DetailSheetSection title={t("deal.whereItStandsTitle")}>
			<DetailSheetProperties>
				<DetailSheetProperty label={t("deal.opened")}>
					<LocalDateTime date={deal.createdAt} options={DATE_OPTIONS} />
				</DetailSheetProperty>

				<DetailSheetProperty label={t("deal.inStageSince")}>
				...
				<DetailSheetProperty label={t("deal.onIt")} wide>
					{deal.contacts.length === 0 ? (
						<span className="text-muted-foreground">
							{t("deal.nobodyAttached", { company: deal.company.name })}
						</span>
					) : (
		...
		return (
			<>
				...
				{adding ? null : (
					<DetailSheetEmpty
						icon={UserMultiple}
						title={t("deal.contactsEmptyTitle")}
						description={t("deal.contactsEmptyDescription", {
							company: deal.company.name,
						})}
						action={
				...
				{deal.contacts.map((contact) => (
					...
					>
						...
						<TableCell className="truncate px-1 py-2.5">
							<InlineTextCell
								label={t("deal.roleForContact", { name: contactName(contact) })}
								value={contact.role}
								...
								<TooltipTrigger asChild>
									...
									>
										...
										<span className="sr-only">
											{t("deal.takeOffDeal", { name: contactName(contact) })}
										</span>
									</Button>
								</TooltipTrigger>
								<TooltipContent>{t("deal.takeOffDealShort")}</TooltipContent>
							</Tooltip>
						</TableCell>
					</SimpleTableRow>
				))}
